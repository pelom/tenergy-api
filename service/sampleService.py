...
class SampleService(object):
    ...
    def get_sample(self, now=datetime.datetime.now()):
        ...
        query = query.filter(Sample.VoltageBattery < 50, Sample.VoltageBattery > 20,
                             Sample.CreatedDate >= start_date, Sample.CreatedDate < end_date)
        battery = query.first()
        ...
        query = query.filter(Sample.CurrentPV > 0, Sample.PowerLowPV > 0, Sample.VoltagePV > 24,
                             Sample.CreatedDate >= start_date, Sample.CreatedDate < end_date)
        pv = query.first()
        ...
        query = query.filter(Sample.CurrentDischarging > 0, Sample.PowerLowDischarging > 0, Sample.VoltageDischarging > 24,
                             Sample.CreatedDate >= start_date, Sample.CreatedDate < end_date)
        load = query.first()
